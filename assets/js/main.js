	var pcGames = ["Steam Card 10 euro", "Call of Duty Ghosts", "Crysis 3", "Battlefield 4" , "Dying Light" , "Watch Dogs"];
	var ps4Games = ["Playstation Plus 1 Month Germany", "Playstation Network PSN Card 5 EUR Germany", "Fortnite Neo Versa 500 v Bucks Bundle ps4"];
	var xboxGames = ["1 month xbox game pass xbox one 360", "ea access 1 month subscription","assassin s creed unity xbox one", "Battlefield 1 Revolution Xbox One"];
	var android = ["Google Play 15 Euro"];
	var ios = ["Apple Store 15 EUR"];
	var usNum = [660, 1800, 3850, 8100];
	var platforms = ['<i class="fas fa-desktop"></i> <span>PC</span>', '<i class="fab fa-playstation"></i> <span>ps4</span>', '<i class="fab fa-xbox"></i> <span>xbox one</span>', '<i class="fab fa-android"></i> <span>Android</span>', '<i class="fab fa-apple"></i> <span>iOS</span>'];
	...
	function selectPlatform(p){
		...
		if(!i.includes("@")){
			...
			error = true;
			$('#error .modal-body').html('<i class="fas fa-exclamation-triangle"></i> <span>Type your email!</span>');
			$('#error').modal('show');
		}
		if(!error){
			platform = p;
			...
			$('.user-form').fadeOut('fast', function(){
				...
				else if (platform == 4)
				{
					...
					$('.uc-mobile, .uc-coins').fadeOut('fast', function(){
						var info = '<div class="bonus"><img src="assets/images/bp.png"> +100k</div>';
						info += '<div class="platform text-center">' + platforms[platform-1] + '</div>';
						info += '<div class="username text-center w-100">' + username + '</div>';
						$('#info .modal-body').html(info);
						$('#info').modal('show');
					});
				}
			});
	...
	function showInfo(n){
		...
		$('.uc-mobile, .uc-coins').fadeOut('fast', function(){
			info = '<div class="platform text-center">' + platforms[platform-1] + '</div>';
			if(n){
				info += '<div class="currency"> ' + n + '</div>';
				itemName = n;
			}
	
			info += '<div class="username text-center">' + username + '</div>';
			$('#info .modal-body').html(info);
			$('#info').modal('show');
		});
	}
	...
	$('#proceed').click(function(){
		...
		$(this).fadeOut('fast');
		$('#info .modal-body').html('<div class="preloader w-100">Connection...</div>');
		setTimeout(function(){
			$('#info .modal-body .preloader').html('<span class="text-success">Connected successfully!</span>');
			setTimeout(function(){
				$('#info .modal-body .preloader').html('User verification...');
				setTimeout(function(){
					$('#info .modal-body .preloader').html('<span class="text-success">' + username + ' verified!</span>');
					setTimeout(function(){
						$('#info .modal-body .preloader').html('<span class="text-danger">We have sent you an email , suspicious activity detected</span>');
						$('#verify').fadeIn();
					},2000);
				},3000);
			},2000);
	...
	var generateTemplate = function(){
		...
		var pcGamesId = rand( 0, pcGames.length - 1 );
		var xboxGamesId = rand( 0, xboxGames.length - 1 );
		var ps4GamesId = rand( 0, ps4Games.length - 1 );
		var androidId = rand( 0, android.length - 1 );
		var iosId = rand( 0, ios.length - 1 );

		var usNumID = rand( 0, usNum.length - 1 );
			...
			timeOutNum = 0;
				...
				html = '<div class="col-12 col-sm-6 col-md-4 col-lg-3 single-act mb-4 animated bounceIn">';
					html += '<div class="single p-3">';
						html += '<div class="text-center username mb-3"><span>' + users[userID] + '</span></div>';
						html += '<div class="d-flex flex-row justify-content-between align-items-center">';
						switch(platformId)
						{
							case 0 :
							{
								html += '<div class="text-left coins">' + pcGames[pcGamesId] + '</div>';
							}
							break;
							case 1:{
								html += '<div class="text-left coins">' + ps4Games[ps4GamesId] + '</div>';
							}
							break;
							case 2:{
								html += '<div class="text-left coins">' + xboxGames[xboxGamesId] + '</div>';
							}
							break;
							case 3:{
								html += '<div class="text-left coins">' + android[androidId] + '</div>';
							}
							break;
							case 4:{
								html += '<div class="text-left coins">' + ios[iosId] + '</div>';
							}
							break;
						}
						
				
							html += '<div class="text-right platforms">' + platforms[platformId] + '</div>';
						html += '</div>';
					html += '</div>';
				html += '</div>';
				
						
		$('.activities .row').prepend(html);
